    ...
    return new Promise(resolve => {
        if (products) {
            ...
            products.forEach(product => {
                if (product.store == newProduct.store && product.name == newProduct.name) {
...
function getUniqueProducts() {
    return new Promise(async resolve => {
        ...
        if (chromeData.products) {
            ...
            // Filter the product based on the same store:productName:link
            let tempUniqueMap = new Map();
            unique = unique.filter(product => {
                let val = tempUniqueMap.get(`${product.store}:${product.name}:${product.link}`);
                if (!val) {
                    tempUniqueMap.set(`${product.store}:${product.name}:${product.link}`, product.name);
                    return true;
                } else {
                    return false;
                }
                ...
                for (let infoType of PRODUCT_STORAGE_CATEGORIES) {
                    if (products[infoType]) {
                        for (productKey in products[infoType]) {
                            ...
                            if (product && ((productObject && product.name == productObject.name && product.store == productObject.store && product.link == productObject.link) || all)) {
                                product[key] = value;
                            }
                        }
                    }
...
/**
 * Returns the value of the matching (by regex) object key
 * @param {json} obj 
 * @param {regex} regex 
 */
function getPropertyByRegex(obj, stringValue) {
    for (key in obj) {
        let storeRegex = new RegExp("^" + key, "gi");
        if (storeRegex.test(stringValue))
            return obj[key];
    }
    return undefined;
}
...
function getUid() {
    return Math.random().toString(36).substr(2, 5).concat(Date.now());
}